...
- 给模型广场、定价页、排行榜提供展示信息
- 支持按供应商筛选模型
- 作为独立元数据表被后台单独维护和同步，而不只是 `model` 的附属字段来源

也就是说，`vendor` 解决的是“这个模型是谁家的”。

### `vendor` 不是“只是给 models 提供预设来源”

如果只把 `vendor` 理解成“给 `models` 提供一个预设来源”，这个理解只对了一部分，但不完整。

更准确地说：

- `vendor` 确实会参与 `model` 元数据的补齐与归属
- 但它本身也是一张独立的元数据表，有自己的增删改查、同步和展示用途
- 它表达的是“模型归属到哪家厂商”，而不是“模型价格或默认倍率从哪里来”
...
- 上游 `vendors.json` / `models.json` 提供的是“模型与厂商元数据资料”
- 它们不是系统默认价格、倍率或配额规则的主要来源
- `vendor` 同步的重点是名称、描述、图标、状态、归属关系这类展示和识别信息
...
- `vendor` 主要回答“这个模型是谁家的”
- `model` 主要回答“系统如何描述和展示这个模型”
- `model_ratio` / `options` 才主要回答“这个模型默认怎么计费”
