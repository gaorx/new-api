...
如果用一句用户视角的话概括，这里的 OAuth 主要就是：

- 用其他网站的账号登录本项目的网站
- 或者把其他网站账号绑定到本项目已有账户

也就是说，这部分 OAuth 的核心语义是“外部身份系统为本平台账户体系提供登录凭据”，而不是“relay 调上游模型时的渠道认证”。
...
这类 OAuth 不面向普通终端用户登录控制台。它面向的是管理员配置 channel 时的上游接入动作，本质上是在为 relay 获取和维护“可调用上游模型接口的凭证”。
...
它的工作原理不是“服务端省掉密码校验”，而是：

1. 用户先在浏览器里发起 Passkey 注册。
2. 浏览器通过 WebAuthn API 调用系统认证器或安全密钥，生成一对密钥。
3. 私钥保留在设备安全区域，本系统只保存公钥和凭证元数据到 `passkey_credentials`。
4. 后续登录或二次验证时，服务端先下发一次性 challenge。
5. 浏览器再次调用 WebAuthn API，让系统认证器用本地私钥对 challenge 签名。
6. 服务端再用数据库里保存的公钥验签，通过后建立或提升会话状态。

因此它本质上是“浏览器标准能力 + 设备认证器 + 服务端验签”的组合，不是一个纯 API 层能力。

这里的 WebAuthn API 指浏览器标准接口，属于 W3C WebAuthn / FIDO2 体系，典型入口是：

- `navigator.credentials.create()`：用于注册 Passkey
- `navigator.credentials.get()`：用于登录或二次验证

这说明本项目里的 Passkey 主要面向 Web 控制台登录与安全验证场景。后端 API 承担的是 challenge 生成、凭证保存、结果校验和 session 建立职责，而不是独立替代浏览器完成整套 Passkey 交互。
...
1. 用户先完成账号密码校验。
2. 如果账户启用了 2FA，再提交认证器 App 生成的 TOTP 动态码，或者提交一次性备用恢复码。

之所以需要 `two_fa_backup_codes`，是因为用户可能在真实场景里失去对认证器的访问能力，比如丢手机、重装认证器、换机未迁移密钥。这时如果没有恢复码，用户会被第二步验证永久挡在门外。

因此 backup code 的设计目标不是“替代 2FA”，而是“在认证器不可用时，提供一次性、低频、可审计的应急恢复入口”。
